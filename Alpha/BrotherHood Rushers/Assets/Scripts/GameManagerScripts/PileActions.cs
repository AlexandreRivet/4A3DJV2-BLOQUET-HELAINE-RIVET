...
public class PileActions {
	...
	public int comparePositionActionToAllActionInPile(float position){
		int index = 0;
		for(index = 0; index < _pileActionPlayer.Count; index++){
			if(_pileActionPlayer[index].comparePositionAction(position, 1f)==true){
				return index;
			}
		}
		return -1;
